...
public class LineRender {
    public static void drawThin(VertexConsumer c, PoseStack.Pose pose, BlockOutlineBuilder.Line line) {
        drawNativeLine(c, pose, line, 1f);
    }

    public static void drawThick(VertexConsumer c, PoseStack.Pose pose, BlockOutlineBuilder.Line line) {
        drawNativeLine(c, pose, line, 5f);
    }

    private static void drawNativeLine(VertexConsumer c, PoseStack.Pose pose, BlockOutlineBuilder.Line line, float width) {
        var a = line.from();
        var b = line.to();
        var col = line.color();
    ...
    private static void vertex(VertexConsumer c, PoseStack.Pose pose, Vec3 v, BlockOutlineBuilder.Color col) {
