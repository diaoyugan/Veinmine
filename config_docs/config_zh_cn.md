...
| 键名 | 类型 | 作用范围 | 默认值 | 说明 |
|---|---|---|---|---|
| `distinguishCropMaturity` | 布尔 | 服务端 | `false` | 是否将 `minecraft:crops` 标签中的成熟与未成熟作物视为不同目标。只区分成熟状态，不逐级区分每个生长阶段。 |
| `distinguishDyedBlockColors` | 布尔 | 服务端 | `true` | 是否区分完整 16 色染色方块系列的颜色。关闭后同系列的不同颜色会被视为相同目标。 |
| `distinguishDeepslateOres` | 布尔 | 服务端 | `false` | 是否将同种普通矿石与深层矿石视为不同目标。关闭后矿脉可以跨越普通石层与深板岩层边界。 |
...
`bridgeOneBlockGap`（布尔，服务端，默认 `false`）：允许 BFS 连锁搜索沿一个方向跳过一个非目标方块，并从两格外的匹配目标继续搜索。被跳过的方块不会被破坏。
